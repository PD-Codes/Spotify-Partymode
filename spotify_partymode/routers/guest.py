...
async def get_state() -> dict:
    ...
    if spotify_client.is_admin_authenticated():
        ...
        if cp:
            current = _simplify(cp, party_on)
        upcoming = [_simplify(t, party_on) for t in queue.get("queue", [])[:12]]
...
async def get_play_history() -> dict:
...
def _simplify(item: dict, party_on: bool = False) -> dict:
    ...
    # Skipping only happens while party mode is on, so only flag it then.
    blacklisted = bool(party_on and track_id and db.is_blacklisted(track_id, artist_ids))
    return {
        "uri": item.get("uri"),
        "name": item.get("name"),
